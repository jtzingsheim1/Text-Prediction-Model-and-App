...
[3]: xxxxx "Wikipedia for _____ effect"

```{r unigrams.3, echo = FALSE, cache = TRUE, fig.align = "center"}

# Prepare better variable names for the table
unigram.labels <- c("Word", "Frequency", "Rank", "Document Frequency")
# Display the top ten unigrams and frequencies
unigram %>%
  select(feature:docfreq) %>%
  slice(1:10) %>%
  kable(col.names = unigram.labels) %>%
  kable_styling(full_width = FALSE)

rm(cumulative.table, unigram, unigram.labels, fractions, occurances,
   unique.words, words.needed, FindWordsNeeded)
...
## Positional Analyis (Bigrams and Trigrams)
